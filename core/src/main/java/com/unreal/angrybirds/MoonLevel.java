...
public class MoonLevel  implements Screen, Serializable {
    private Main Game;
...
//        Scorefont.getData().setScale(1.2f);
        try{
            serializedLevel = Game.loadGameScreen("MoonLevel");
        }catch(Exception e){
            Gdx.app.log("loadMessage", "Game State Not Found!!");
        }
        if (serializedLevel instanceof MoonLevel) {
            MoonLevel level = (MoonLevel) serializedLevel;
            player = level.player;
...
//    }
    ...
    public void endGame(){
        if (player.getScore() >= allPigScore && allPigDead()) {
            if ((SpaceBird == null || !SpaceBird.isItLaunched()) && allBlockRested()) {
                Player oldRecord = Game.loadGameScore("MoonLevelScore");
                if (oldRecord == null || oldRecord.getScore() < player.getScore()) {
                    Game.saveGameScore(player, "MoonLevelScore");
                }
                Game.removeFile("MoonLevel");
                Game.setScreen(new SpaceLevelEnd(Game, player,"Moon"));
            }
        }
    }
    void fastForward(){
    ...
    public void show() {
        ...
        camera.update();
        if (sprite == null) sprite = new Sprite(new Texture("assets/MoonLevel.png"));
        if (batch == null) batch = new SpriteBatch();

        if (world == null) {
            world = new World(new Vector2(0, Game.spaceGravityByPlantName("Moon")), false);
            world.setGravity(new Vector2(0, Game.spaceGravityByPlantName("Moon")));
            world.setContactListener(new CollisionDetector());
            if (PigList == null && !isSerialized) {
                PigList = new ArrayList<Piggy>();
                PigList.add(new Piggy("First Piggy",5, null,"assets/MushPig.png",world,"Moon", (int) (955+47/2f), (int) (720-617-47/2f),47,47,5000));
                PigList.add(new Piggy("Second Piggy",3,null,"assets/ProfPig.png",world,"Moon",(int) (1042+47/2f), (int) (720-441-47/2f),47,47,10000));
                PigList.add(new Piggy("Third Piggy",7,null,"assets/KingPig.png",world,"Moon",(int) (1038+47/2f), (int) (720-537-47/2f),47,57,20000));
//                PigList.add(new Piggy("Fourth Piggy",4,null,"assets/CorpPig.png",world,"Moon",(int) (939+47/2f), (int) (720-615-47/2f),47,43,9000));
                PigList.add(new Piggy("Fifth Piggy",5,null,"assets/FirstPiggy.png",world,"Moon",(int) (1129+47/2f), (int) (720-615-47/2f),47,47,20000));
...
//        Game.clickHandling(Nextbutton, nextButtonPixmap, new SpaceLevelEnd(Game));
        ...
        Game.clickHandling(PauseButton, pauseButtonPixmap, new SpacePauseScreen(Game, "assets/MoonBig.png","Moon", this));
...
//                    world.destroyBody(SpaceBird.getBirdBody());
                    ...
                    if (birdsAvailable <= 0) {
                        ...
                        SpaceBird = new Bird("Red", 20, new WarCryAbility(), "assets/RedBirdMain.png",world,"Moon");
                        birdsAvailable--;
...
//                    world.destroyBody(SpaceBird.getBirdBody());
                    ...
                    if (birdsAvailable <= 0) {
                        ...
                        SpaceBird = new Bird("Chuck", 10, new SpeedAbility(), "assets/YellowBirdMain.png",world,"Moon");
                        birdsAvailable--;
                        BirdX  = SpaceBird.getX();
                        BirdY = SpaceBird.getY();
...
//                    world.destroyBody(SpaceBird.getBirdBody());
                    ...
                    if (birdsAvailable <= 0) {
                        ...
                        SpaceBird = new Bird("Blue", 8, new SplitAbility(), "assets/BlueBirdMain.png",world,"Moon");
                        birdsAvailable--;
                        BirdX  = SpaceBird.getX();
                        BirdY = SpaceBird.getY();
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                if (SpaceBird != null && SpaceBird.isItLaunched()) {
                    ...
                    if (birdsAvailable <= 0) {
                        ...
                        SpaceBird = new Bird("Bomb", 8, new ExplodeAbility(), "assets/BombBirdMain.png",world,"Moon");
                        birdsAvailable--;
                        BirdX  = SpaceBird.getX();
                        BirdY = SpaceBird.getY();
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                if (SpaceBird != null && SpaceBird.isItLaunched()) {
                    ...
                    if (birdsAvailable <= 0) {
                        ...
                        SpaceBird = new Bird("Matilda", 6, new EggAbility(), "assets/WhiteBirdMain.png",world,"Moon");
                        birdsAvailable--;
                        BirdX  = SpaceBird.getX();
                        BirdY = SpaceBird.getY();
...
//        if(birdsAvailable<=0){
//            Game.removeFile("MoonLevel");
////            Game.removeFile("MoonLevelScore");
//            Game.setScreen(new SpaceLevelEnd(Game,player,"Moon"));
//        }
