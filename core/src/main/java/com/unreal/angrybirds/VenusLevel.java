...
public class VenusLevel  implements Screen, Serializable {
    private Main Game;
...
//        Scorefont.getData().setScale(1.2f);
        try{
            serializedLevel = Game.loadGameScreen("VenusLevel");
        }catch(Exception e){
            Gdx.app.log("loadMessage", "Game State Not Found!!");
        }
        if (serializedLevel instanceof VenusLevel) {
            VenusLevel level = (VenusLevel) serializedLevel;
            player = level.player;
...
public void endGame(){
    ...
    public void show() {
        ...
        camera.update();
        if (sprite == null) sprite = new Sprite(new Texture("assets/VenusLevel.png"));
        if (batch == null) batch = new SpriteBatch();

        if (world == null) {
            world = new World(new Vector2(0, -8.87f), false);
            world.setGravity(new Vector2(0, -8.87f));
            world.setContactListener(new CollisionDetector());
            if (PigList == null && !isSerialized) {
                PigList = new ArrayList<Piggy>();
                PigList.add(new Piggy("First Piggy",5, null,"assets/MushPig.png",world,"Venus", (int) (955+47/2f), (int) (720-617-47/2f),47,47,5000));
                PigList.add(new Piggy("Second Piggy",3,null,"assets/ProfPig.png",world,"Venus",(int) (1042+47/2f), (int) (720-441-47/2f),47,47,10000));
                PigList.add(new Piggy("Third Piggy",7,null,"assets/KingPig.png",world,"Venus",(int) (1038+47/2f), (int) (720-537-47/2f),47,57,20000));
//                PigList.add(new Piggy("Fourth Piggy",4,null,"assets/CorpPig.png",world,"Venus",(int) (939+47/2f), (int) (720-615-47/2f),47,43,9000));
                PigList.add(new Piggy("Fifth Piggy",5,null,"assets/FirstPiggy.png",world,"Venus",(int) (1129+47/2f), (int) (720-615-47/2f),47,47,20000));
...
//        Game.clickHandling(Nextbutton, nextButtonPixmap, new SpaceLevelEnd(Game));
        ...
        Game.clickHandling(PauseButton, pauseButtonPixmap, new SpacePauseScreen(Game, "assets/VenusBig.png","Venus", this));
        ...
        RedBirdButton.addListener(new InputListener() {
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (birdsAvailable > 1) SpaceBird = new Bird("Red", 20, new WarCryAbility(), "assets/RedBirdMain.png",world,"Venus");
...
//                birdsAvailable--;
        ...
        YellowBirdButton.addListener(new InputListener() {
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (birdsAvailable > 1) SpaceBird = new Bird("Chuck", 10, new SpeedAbility(), "assets/YellowBirdMain.png",world,"Venus");
                else {flag = false; return true;}
//                birdsAvailable--;
                flag = false;
                BirdX  = SpaceBird.getX();
        ...
        BlueBirdButton.addListener(new InputListener() {
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (birdsAvailable > 1) SpaceBird = new Bird("Blue", 8, new SplitAbility(), "assets/BlueBirdMain.png",world,"Venus");
                else{ flag = false; return true;}
//                birdsAvailable--;
                flag = false;
                BirdX  = SpaceBird.getX();
...
//
//                }
//                SpaceBird = new Bird("Bomb", 8, new ExplodeAbility(), "assets/BombBirdMain.png",world,"Venus");
//                birdsAvailable--;
//                BirdX  = SpaceBird.getX();
//                BirdY = SpaceBird.getY();
//                return true;
...
//                    world.destroyBody(SpaceBird.getBirdBody());
//                }
//                SpaceBird = new Bird("Matilda", 6, new EggAbility(), "assets/WhiteBirdMain.png",world,"Venus");
//                birdsAvailable--;
//                BirdX  = SpaceBird.getX();
//                BirdY = SpaceBird.getY();
//                return true;
...
//        world = SpaceBird.getWorldInstance();
    ...
    public void render(float delta) {
        ...
        if(birdsAvailable<=0){
            Game.removeFile("VenusLevel");
//            Game.removeFile("VenusLevelScore");
            Game.setScreen(new SpaceLevelEnd(Game,player,"Venus"));
        }
