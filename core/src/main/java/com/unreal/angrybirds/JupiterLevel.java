...
public void endGame(){
    ...
    public void show() {
        ...
        if (world == null) {
            world = new World(new Vector2(0, -23.1f), false);
            world.setGravity(new Vector2(0, -23.1f));
...
//        Game.clickHandling(Nextbutton, nextButtonPixmap, new SpaceLevelEnd(Game));
        ...
        RedBirdButton.addListener(new InputListener() {
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (birdsAvailable > 1) SpaceBird = new Bird("Red", 20, new WarCryAbility(), "assets/RedBirdMain.png",world,"Jupiter");
...
//                birdsAvailable--;
        ...
        YellowBirdButton.addListener(new InputListener() {
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (birdsAvailable > 1) SpaceBird = new Bird("Chuck", 10, new SpeedAbility(), "assets/YellowBirdMain.png",world,"Jupiter");
                else {flag = false; return true;}
//                birdsAvailable--;
                flag = false;
                BirdX  = SpaceBird.getX();
        ...
        BlueBirdButton.addListener(new InputListener() {
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (birdsAvailable > 1) SpaceBird = new Bird("Blue", 8, new SplitAbility(), "assets/BlueBirdMain.png",world,"Jupiter");
                else{ flag = false; return true;}
//                birdsAvailable--;
                flag = false;
                BirdX  = SpaceBird.getX();
