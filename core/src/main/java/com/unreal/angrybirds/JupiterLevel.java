...
public class JupiterLevel  implements Screen, Serializable {
    private Main Game;
...
//        Scorefont.getData().setScale(1.2f);
        try{
            serializedLevel = Game.loadGameScreen("JupiterLevel");
        }catch(Exception e){
            Gdx.app.log("loadMessage", "Game State Not Found!!");
        }
        if (serializedLevel instanceof JupiterLevel) {
            JupiterLevel level = (JupiterLevel) serializedLevel;
            player = level.player;
...
// Pass the scaled deltaTime to the physics world
    ...
    public void show() {
        ...
        camera.update();
        if (sprite == null) sprite = new Sprite(new Texture("assets/JupiterLevel.png"));
        if (batch == null) batch = new SpriteBatch();

        if (world == null) {
            world = new World(new Vector2(0, Game.spaceGravityByPlantName("Jupiter")), false);
            world.setGravity(new Vector2(0, Game.spaceGravityByPlantName("Jupiter")));
            world.setContactListener(new CollisionDetector());
            if (PigList == null && !isSerialized) {
                PigList = new ArrayList<Piggy>();
                PigList.add(new Piggy("First Piggy",5, null,"assets/MushPig.png",world,"Jupiter", (int) (955+47/2f), (int) (720-617-47/2f),47,47,5000));
                PigList.add(new Piggy("Second Piggy",3,null,"assets/ProfPig.png",world,"Jupiter",(int) (1042+47/2f), (int) (720-441-47/2f),47,47,10000));
...
//        Game.clickHandling(Nextbutton, nextButtonPixmap, new SpaceLevelEnd(Game));
        ...
        Game.clickHandling(PauseButton, pauseButtonPixmap, new SpacePauseScreen(Game, "assets/JupiterBig.png","Jupiter", this));
...
//                    world.destroyBody(SpaceBird.getBirdBody());
                    ...
                    if (birdsAvailable <= 0) {
                        ...
                        SpaceBird = new Bird("Red", 20, new WarCryAbility(), "assets/RedBirdMain.png",world,"Jupiter");
                        birdsAvailable--;
...
//                    world.destroyBody(SpaceBird.getBirdBody());
                    ...
                    if (birdsAvailable <= 0) {
                        ...
                        SpaceBird = new Bird("Chuck", 10, new SpeedAbility(), "assets/YellowBirdMain.png",world,"Jupiter");
                        birdsAvailable--;
                        BirdX  = SpaceBird.getX();
                        BirdY = SpaceBird.getY();
...
//                    world.destroyBody(SpaceBird.getBirdBody());
                    ...
                    if (birdsAvailable <= 0) {
                        ...
                        SpaceBird = new Bird("Blue", 8, new SplitAbility(), "assets/BlueBirdMain.png",world,"Jupiter");
                        birdsAvailable--;
                        BirdX  = SpaceBird.getX();
                        BirdY = SpaceBird.getY();
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                if (SpaceBird != null && SpaceBird.isItLaunched()) {
                    ...
                    if (birdsAvailable <= 0) {
                        ...
                        SpaceBird = new Bird("Bomb", 8, new ExplodeAbility(), "assets/BombBirdMain.png",world,"Jupiter");
                        birdsAvailable--;
                        BirdX  = SpaceBird.getX();
                        BirdY = SpaceBird.getY();
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                if (SpaceBird != null && SpaceBird.isItLaunched()) {
                    ...
                    if (birdsAvailable <= 0) {
                        ...
                        SpaceBird = new Bird("Matilda", 6, new EggAbility(), "assets/WhiteBirdMain.png",world,"Jupiter");
                        birdsAvailable--;
                        BirdX  = SpaceBird.getX();
                        BirdY = SpaceBird.getY();
...
//        if(birdsAvailable<=0){
//            Game.removeFile("JupiterLevel");
////            Game.removeFile("JupiterLevelScore");
//            Game.setScreen(new SpaceLevelEnd(Game,player,"Jupiter"));
//        }
